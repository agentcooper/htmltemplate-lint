...
Content = (Comment / SingleTag / BlockTag / ConditionalTag / InvalidTag / Text)*
...
SingleTag = OpeningBracket name:SingleTagName attributes:Attributes* ClosingBracket {
  return token({
...
InvalidTag = (OpeningEndBracket / OpeningBracket) name:$(!KnownTagName "TMPL_" [a-zA-Z_]+) attributes:Attributes* ClosingBracket {
  return token({
...
EndTag = OpeningEndBracket name:BlockTagName ClosingBracket {
  return name;
}
...
SingleLineComment = CommentStart c:$(!LineTerminator SourceCharacter)* LineTerminator {
  return token({
...
FullLineComment = FullLineCommentStart c:$(!LineTerminator SourceCharacter)* LineTerminator {
  return token({
    type: BLOCK_TYPES.COMMENT,
    content: c
  }, line, COLUMN_ONE);
...
Attributes = WhiteSpace+ attrs:Attribute {
  return attrs;
}
...
AttributeWithValue = name:AttributeName "=" value:(PlainAttributeValue / PerlExpression / QuotedString) {
  return token({
...
PlainAttributeValue = v:$[a-zA-Z0-9\-_/]+ {
  return v;
}
